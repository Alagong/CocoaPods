    ...
    class Analyzer
      ...
      def generate_pod_targets(specs_by_target)
        if config.deduplicate_targets?
          distinct_targets = specs_by_target.flat_map do |target_definition, specs|
            specs.group_by(&:root).map { |root_spec, specs| [root_spec, specs, target_definition] }
          end.each_with_object({}) do |dependency, hash|
